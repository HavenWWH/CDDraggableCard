...
@interface CDDraggableContainer ()
...
@implementation CDDraggableContainer
...
- (void)awakeFromNib {
    ...
    [super awakeFromNib];
    self.style = CDDraggableStyleUpOverlay;
}
...
- (void)defaultConfig {
    ...
    self.direction = CDDraggableDirectionDefault;
    self.loadedIndex = 0;
    self.moving = NO;
}
        ...
        */
        ...
        if (self.loadedIndex < indexs) {
            ...
            for (long int i = self.currentCards.count; i <  (self.moving ? preloadViewCont + 1: preloadViewCont); i++) {
                ...
                CDDraggableCardView *cardView = [self.dataSource draggableContainer:self viewForIndex:self.loadedIndex];
...
- (void)tapGestureHandle:(UITapGestureRecognizer *)tap {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:cardView:didSelectIndex:)]) {
        [self.delegate draggableContainer:self cardView:(CDDraggableCardView *)tap.view didSelectIndex:tap.view.tag];
    }
}
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        CDDraggableCardView *cardView = (CDDraggableCardView *)gesture.view;
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(draggableContainer:draggableDirection:widthRatio:heightRatio:)]) {
            ...
            */
            ...
            if (widthRatio > 0) {
                self.direction = CDDraggableDirectionRight;
            } if (widthRatio < 0) {
                self.direction = CDDraggableDirectionLeft;
...
- (void)finishedPanGesture:(UIView *)cardView direction:(CDDraggableDirection)direction scale:(CGFloat)scale disappear:(BOOL)disappear {
    ...
    */
    ...
    if (!disappear) {
        ...
        */
        ...
        NSInteger flag = direction == CDDraggableDirectionLeft ? -1 : 2;
        [UIView animateWithDuration:0.5f
                              delay:0.0
                            options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionAllowUserInteraction
                         animations:^{
                             cardView.center = CGPointMake(CDDragWidth * flag, CDDragWidth * flag / scale + self.cardCenter.y);
...
- (void)removeForDirection:(CDDraggableDirection)direction {
    ...
    if (self.moving) {
        ...
        switch (direction) {
            case CDDraggableDirectionLeft:
                cardCenter = CGPointMake(-CDDragWidth / 2, self.cardCenter.y);
                flag = -1;
                break;
            case CDDraggableDirectionRight:
                cardCenter = CGPointMake(CDDragWidth * 1.5, self.cardCenter.y);
                flag = 1;
                break;
            default:
                break;
...
- (void)resetVisibleCards {
    ...
    __weak CDDraggableContainer *weakself = self;
...
- (void)movingVisibleCards:(CGFloat)scale {
    ...
    for (int i = 1; i < self.currentCards.count; i++) {
        ...
        CDDraggableCardView *cardView = [self.currentCards objectAtIndex:i];
